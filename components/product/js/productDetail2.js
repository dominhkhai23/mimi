...
var productEye = document.querySelectorAll('.content_index2-product-eye-link')

productEye.forEach( (product, index) => {
    product.onclick = function() {
        var data = Array.from(dataImg).filter((img, index2) => {
            return index === index2
        })
        window.localStorage.removeItem('sampham')
        window.localStorage.setItem('sanpham', JSON.stringify(data))
        product.href = '../productDetail/productDetail.html'
    }
})
